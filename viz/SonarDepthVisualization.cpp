...
SonarDepthVisualization::SonarDepthVisualization()
{
    newPoints = false;
 ...
 * @return main node
 */
osg::ref_ptr< osg::Node > SonarDepthVisualization::createMainNode()
{
...
QColor SonarDepthVisualization::getDefaultFeatureColor()
{
    QColor color;
...
void SonarDepthVisualization::setDefaultFeatureColor(QColor color)
{
...
void SonarDepthVisualization::updateDataIntern(const std::vector< base::Vector3d >& data)
{
    channelInfos = data;
}
...
{/*
    if(newPoints)
    {
        ...
        for(std::vector<base::Vector3d>::const_iterator pos = pointCloud.points.begin(); pos != pointCloud.points.end(); pos++)
        {
            ...
            pointsOSG->push_back(vec + osg::Vec3d(0.2, 0.0, 0.0));
	    
	    osg::Vec3d vec2(pos->x(), pos->y() - 0.1 , pos->z());
            pointsOSG->push_back(vec2);
            pointsOSG->push_back(vec2 + osg::Vec3d(0.0, 0.2, 0.0));
            
	    
            if(chan_pos != channelInfos.end())
            {
                color->push_back(osg::Vec4(chan_pos->z(), chan_pos->z(), chan_pos->z(), 1.0f));
                chan_pos++;
            }
            else
            {
	    ...
	    //color->push_back(osg::Vec4(0.0, 0.0, pos->z(), pos->z()/20.0));
	    //color->push_back(osg::Vec4(0.0, 0.0, pos->z(), pos->z()/20.0));
        }
